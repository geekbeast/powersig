...
def torch_compute_differences(X: torch.Tensor, y: torch.Tensor = None) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
    """
...
def scale_and_shift(delta_X: torch.Tensor, delta_y: Optional[torch.Tensor] = None, scaling_type="channel", scale: Union[float, torch.Tensor, None] = None) -> Union[Tuple[torch.Tensor,torch.Tensor,torch.Tensor], Tuple[torch.Tensor,torch.Tensor,torch.Tensor,torch.Tensor]]:
    """
